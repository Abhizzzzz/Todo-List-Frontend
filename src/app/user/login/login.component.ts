...
export class LoginComponent implements OnInit {
  ...
  onChange(e) {
    ...
    for (let code in this.possibleCode) {
      if (code == this.countryCategory) {
        this.selectedCountryCode = this.possibleCode[code];
  ...
  forgotPassword(): any{
    this.service.sendMail(this.forgetPassEmail).subscribe((data) =>{
      ...
      if(data.status === 200){
        ...
        this.toastr.success("Change password email has been sent to your registered mail id");
      }
      else{
